...
const integerZero = "0";
var moduleId = "";
const numericTextBoxMaxLength = 18;
const intTextBoxMaxLength = 15;
        ...
        setting.Parameters.forEach((param) => {
            ...
            switch (param.ControlType) {
                ...
                case ControlTypeEnum.Grid:
                    ...
                    var htmlString = '<div class="edit-cell input-text" id="{0}" name="{0}" style="width : 92; padding : 2px;"><input name="{1}"id = "{2}" tabindex = "{3}" type = "text" name = "{1}" style = "width : 100%;height:30px;" class="textFinder"/><input type="button" name="{1}"class="icon btn-search" id="{4}" style="right: 35px;background-size:925px 93px;background-color: transparent;border:none" /></div ><div id="{5}" name="{0}" class="datepicker small k-input" style="width : 92; padding : 2px;display:none;"><input type="date" name="{6}" id="{7}" tabindex="{3}" formattextbox="date" maxlength="10" placeholder="M/d/yyyy"style="width : 100%;" class="dateValue"/></div><div id="{8}" name="{0}" style="width : 92; padding : 2px;display:none;"><input id="{9}" name="{10}" tabindex ="{3}" style="width : 100%;height:30px;" /></div><span id="{11}" name="{12}" style="color:black;" />'
                            ...
                            for (var i = 0; i <= girdTotalRows; i++) {
                                ...
                                $(txtGridColOptField).bind('change', function (e) {
                                    ...
                                    sg.delayOnChange(InputControlIdEnum.OptionalButtonField + index, $("#" + InputControlIdEnum.OptionalTextField + index), function () {
                                        _sendRequest(e, changeControlType, i);
                                    });
                                });
                            }
                        }
                    ...
                    function _requestComplete(jsonResult, changeControlType, index) {
                        setOptionalFieldValue(jsonResult.Data, changeControlType, index)
                    }
                 ...
                 */
                    function _sendRequest(e, changeControlType, index) {
                        ...
                        var data = { 'viewID': property.viewID, 'record': { "OPTFIELD": e.target.value, "isGuid": false }, 'fieldName': optFldName },
                            requestName = requestFetch;
                        ...
                        sg.utls.ajaxPostSync(url, data, function (jsonResult) {
                            ...
                            if (!isSuccess) {
                                var urlMsg = jsonResult.UserMessage.Errors[0].Message;
                   ...
                   */
                    var optionalFinderInfo = (sourceField, targetField, isNextRowFocus = false) => {
                        $('input[name="' + sourceField + '"]').keypress(function (event) {
                            ...
                            //allowing only the numeric values, Minus and Dot for Amount
                            if ((type == ValueTypeEnum.Amount || type == ValueTypeEnum.Number) && (sourceField == ControlNameEnum.FromColumn || sourceField == ControlNameEnum.ToColumn)) {
                                KeyRestriction(event);
                            }
                            //allowing the Minus and numeric for Integer and Number
                            else if ((type == ValueTypeEnum.Integer) && (sourceField == ControlNameEnum.FromColumn || sourceField == ControlNameEnum.ToColumn)) {
                                ...
                                if (keyCodes.indexOf(event.keyCode) > -1 ||
                                    (event.keyCode == Keycodes.KeyValueA && event.ctrlKey === true) ||
                                    (event.keyCode >= Keycodes.End && event.keyCode <= Keycodes.ArrowRight)) {
                                    return;
                                }
                                else {
                                    if (event.shiftKey || ((event.keyCode != Keycodes.Plus && event.keyCode < Keycodes.Zero) || event.keyCode > Keycodes.Nine) && (event.keyCode < Keycodes.NumpadZero || event.keyCode > Keycodes.NumPadNine) && (event.keyCode != Keycodes.Backspace && event.keyCode != Keycodes.Delete)) {
                                        event.preventDefault();
                                    }
                                }
                            }
                            //restrict the alphabets and special characters for Time
                            else if (type == ValueTypeEnum.Time && (sourceField == ControlNameEnum.FromColumn || sourceField == ControlNameEnum.ToColumn)) {
                        ...
                        */
                        ...
                        switch (optionalFieldType) {
                            ...
                            case ValueTypeEnum.Amount:
                                ...
                                if (optionalFieldType == ValueTypeEnum.Integer || optionalFieldType == ValueTypeEnum.Number || optionalFieldType == ValueTypeEnum.Amount) {
                                    formattedTextBox(index, optionalFieldType);
                                }
                                else if (optionalFieldType == ValueTypeEnum.Time) {
                                    formattedTextBox(index, optionalFieldType);
                                }
                                else {
...
function setFinderEvent(i, changeControlType) {
    ...
    sg.viewFinderHelper.setViewFinder(InputControlIdEnum.OptionalButtonField + i,
        function (result, sender) {
            if (result) {
                setOptionalFieldValue(result, changeControlType, i)
            }
        }, optionalFinderInfo);
}
...
/**@name setOptionalFieldValue
   ...
   @param { number } index row index
   */
function setOptionalFieldValue(result, changeControlType, index) {
    if (result) {
...
function gridControlsValidations() {
    ...
    /**@name optTextValue
       ...
       @param { string } fromDateValue From Value for Date
       @param { string } toDateValue To Value for Date
     */
    function setValueToParameters(fromValue, toValue, optFldType, fromDateValue = "", toDateValue = "") {
        if (optFldType == ValueTypeEnum.Date) {
            $("#" + HdnFieldEnum.FromFilterValue + j).val(fromDateValue);
            $("#" + HdnFieldEnum.ToFilterValue + j).val(toDateValue);
        }
        else {
    ...
    function formatDecimalValue(decimalValue, defaultValue, optFldType) {
        ...
        setValueToParameters(fromFieldValue, toFieldValue, optFldType);
    }
    ...
    const maxNumberValue = "999999999999999.999"; //Max value for Number Textbox
        ...
        switch (type) {
            ...
            case ValueTypeEnum.YesNo:
                ...
                if (fromValue > toValue || (fromValue != "") && (toValue == "")) {
                    ...
                    setValueToParameters(fromValue, toValue, ValueTypeEnum.YesNo);
                }
                break;
            case ValueTypeEnum.Amount:
            case ValueTypeEnum.Number:
                if ((fromValue > maxNumberValue)) {
                    invalidEntry = "InvalidFromNumberFormatting";
                }
                else if ((toValue > maxNumberValue)) {
                    invalidEntry = "InvalidToNumberFormatting";
                }
            ...
            case ValueTypeEnum.Integer:
            case ValueTypeEnum.Time:
                if (parseInt(fromValue) > parseInt(toValue) || (fromValue != "") && (toValue == "")) {
                    invalidEntry = "InvalidEntryMessage";
                }
                else {
                    if (type == ValueTypeEnum.Integer)
                        ...
                        setValueToParameters(sg.utls.checkIfValidTimeFormat(fromValue), sg.utls.checkIfValidTimeFormat(toValue), ValueTypeEnum.Time);
                    }
                }
                break;
        }
